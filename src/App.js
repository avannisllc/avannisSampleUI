...
class App extends React.Component { 
  constructor(props){ 
    ...
    this.state = {
      ...
      form_datas: [],
      interviewIndex: 0,
      interviewGroup: [<div key={0} style={{textAlign: 'center', fontSize: 24, fontWeight: 'bold', marginRight: 15, marginTop: 15, marginBottom: 20, color: '#233d4f'}}>Please select a date range.</div>],
      lastInterview: false,
      firstInterview: true,
      quotaGrid: [],
      quotaGridCopy: [],
  ...
  async fetchBanks(e) {
    ...
    if (bank_name.length > 0 || name.length > 0) {
      let listBanksApiData = await API.graphql({ query: listBanks , variables: {filter: {bank_name: {contains: bank_name}, active: {eq: true}}}})
      ...
      this.setState({
        banks: listBanksApiData.data.listBanks.items, showResults: true,
        display_map: {
          "top_row": topRow,
          "groupings": groupings,
  ...
  async fileHandler(fileName, option){
    ...
    const client = new S3Client({credentials: config, region: awsconfig.aws_project_region, Bucket: "https://avannis-data-processing.s3.us-west-2.amazonaws.com/"});
    let command
    if (option === 'delete'){
      let key = "data-pre-processing/rejected/evolytics1/" + fileName.name
      let delete_log_file = fileName.delete_log_file.slice(fileName.delete_log_file.indexOf('data-pre-processing'), fileName.delete_log_file.length) || ''
      let params = {
      ...
      fileName.forEach(file => {
        ...
        if (file.delete_log_file){
          let delete_log_file = file.delete_log_file.slice(file.delete_log_file.indexOf('data-pre-processing'), file.delete_log_file.length)
          params.Delete.Objects.push({Key: delete_log_file})
        }
      })
      command = new DeleteObjectsCommand(params);
  ...
  async getInterviews(bankName, startDate, endDate, includeUpdatedInts) {
    ...
    if (includeUpdatedInts){
      interviews = await API.graphql({ query: listInterviews, variables : { filter: {bank_name: {eq: bankName}, survey_date_submitted: {gt: startDate, lt: endDate}}}})
    } else {
      interviews = await API.graphql({ query: listInterviews, variables : { filter: {bank_name: {eq: bankName}, survey_date_submitted: {gt: startDate, lt: endDate}, updated_count: {eq: 0}}}})
    }
    ...
    await axios
      ...
      .then((response) => {
        ...
        this.setLoadedBar()
          ...
          document.querySelector('.progress-bar.progress-bar-animated.progress-bar-striped').style.backgroundColor = "#C81F28"
          setTimeout(() => {document.getElementById('progressBar').style.zIndex = -10}, 2500)
          setTimeout(() => {this.unSetLoadedBar()}, 3500)
      ...
      try {
        setTimeout(() => { document.querySelector('.progress-bar.progress-bar-animated.progress-bar-striped').style.backgroundColor = "#697582"}, 5000)
      } catch {
      }
  ...
  async submitInterview() {
    ...
    await this.getInterviews(this.state.banks[0].bank_name, this.state.startDate, this.state.endDate, this.state.includeUpdatedInts)
    ...
    } else if (this.state.interviewIndex === this.state.interviewGroup.length && this.state.interviewGroup.length > 1){
      this.setState({
        interviewIndex: this.state.interviewIndex - 1
      })
    }
  ...
  allFileSelectHandler(file, selectAllChecked, fileClicked) {
      ...
      fileIdLength > 0 ? index = this.state.selectedOptions.indexOf(file.id) : index = this.state.selectedOptions.indexOf(file.value)
  ...
  textAreaChange(e, key, initialText){
    ...
    if (textAreaArrFlat.length === 0) {
      ...
      if (this.state.formCheckboxes.length === 0 && this.state.textAreas.length === 1 && e.target.value === initialText) {
        this.setState({
          textAreas: [],
          disabled: true
        })
  ...
  formCheckboxChange(e, key, startCheckValue){
    ...
    if (checkboxesFlat.length === 0){
      ...
      if (this.state.textAreas.length === 0 && this.state.formCheckboxes.length === 1 && startCheckValue === e.target.checked){
        this.setState({
          formCheckboxes: [],
          disabled: true
        })
  ...
  createForm(interviews, map){
    ...
    if (interviews[0].bank_name !== 'nodata') {
      ...
      arr.forEach((sortedForm, index) => {
          ...
          test.forEach((item, index) => {
            topRow.forEach(searchItem => {
              ...
              if (this.searchItem(item, searchItem)) {
                if (item.props.children.length > 0 && item.props.children[1].props.type === "checkbox"){
                ...
                comboTopRowDiv = <div key={searchItem + index + extraKey} style={{paddingTop: 15, position: "sticky", top: 0, paddingBottom: 30, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: "center", backgroundColor: "rgb(239, 239, 239)", borderBottom: "5px solid #233d4f", zIndex: 100}} className='form-group'>{topRowDiv}</div>
              }
            })
          })
        }
        ...
        displayOrder.forEach((displayItem, idx) => {
          test.forEach((item, index) => {
            ...
            if (item.key.includes(displayItem) && (displayItem === item.key.substring(0, item.key.indexOf(displayItem) + displayItem.length) || displayItem === item.key)){
              newOrder.push(item) 
            }
          })
        })
        ...
        newOrder.forEach((item, index) => {
          groups.forEach(groupArr => {
            ...
            if ((item.key === groupVal || groupVal === item.key.substring(0, item.key.indexOf(groupVal) + groupVal.length))){
              groupAmount += 1
        ...
        for (let key in sortedGroups){
          if (sortedGroups[key].length > 1){
            ...
            sortedGroups[key].forEach((item, index) => {
              if (index === 0){
                formGroupTop.push(<div key={item.key + index} style={{marginTop: -20, paddingBottom: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: "center"}} className='form-group'>{item.props.children}<div style={{display: 'flex'}}>{formGroupUnderItems}</div></div>)
              } else {
      ...
      returnDiv.push([<div key={0} style={{textAlign: 'center', fontSize: 24, fontWeight: 'bold', marginRight: 15, marginTop: 15, marginBottom: 20, color: '#233d4f'}}>No interviews available. Please select a different date range.</div>])
    }
    this.setState({
      testForm: returnDiv,
      returnDiv:returnDiv 
       ...
       <div className="App" style={{backgroundColor: '#f8fcff'}}>
        <div style={{height: '100%'}}>
          <Header 
            ...
            />
          <div style={{height: 'inherit', width: 'auto', backgroundColor: 'rgb(248, 252, 255)', display: 'inline-block'}}>
            <Bank  
              ...
              startDate={this.state.startDate} endDate={this.state.endDate} setDateRange={this.setDateRange.bind(this)} 
              ...
              selectAllChecked={this.state.selectAllChecked} 
              bank={this.state.bankInfo} selectedBank={this.state.selectedBank} 
              ...
              open={this.state.open} data={this.state.data}/>
              </div>
          </div>
          <div className='footerWrapper'>
          <div id='bottom' className='footer'></div>
        </div>
      </div>
    )};
}
